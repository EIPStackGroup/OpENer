...
void ConnectionObjectInitializeFromMessage(
  ...
  /* keep it to none existent till the setup is done this eases error handling
   * and the state changes within the forward open request can not be detected
   * from the application or from outside (reason we are single threaded)
   * */
  ConnectionObjectSetState(connection_object,
                           kConnectionObjectStateNonExistent);
  ...
  /* the requested packet interval parameter needs to be a multiple of TIMERTICK
   * from the header file */
  OPENER_TRACE_INFO("ForwardOpen: ConConnID %" PRIu32 ", ProdConnID %" PRIu32
                    ", ConnSerNo %u\n",
...
void ConnectionObjectSetState(CipConnectionObject* const connection_object,
  ...
  switch (state) {
    case kConnectionObjectStateNonExistent:
      connection_object->state = CIP_CONNECTION_OBJECT_STATE_NON_EXISTENT;
      break;
    case kConnectionObjectStateConfiguring:
      connection_object->state = CIP_CONNECTION_OBJECT_STATE_CONFIGURING;
      break;
    ...
    case kConnectionObjectStateTimedOut:
      connection_object->state = CIP_CONNECTION_OBJECT_STATE_TIMEOUT;
      break;
    case kConnectionObjectStateDeferredDelete:
      connection_object->state = CIP_CONNECTION_OBJECT_STATE_DEFERRED_DELETE;
      break;
    case kConnectionObjectStateClosing:
      connection_object->state = CIP_CONNECTION_OBJECT_STATE_CLOSING;
      break;
    default:
