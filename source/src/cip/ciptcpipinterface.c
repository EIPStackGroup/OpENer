...
#define CFG_CAPS_CFG_CHG_NEEDS_RESET \
  0x40U /**< Interface configuration change needs RESET */
#define CFG_CAPS_ACD_CAPABLE 0x80U /**< Device supports ACD */

/* OPENER_TCPIP_IFACE_CFG_SETTABLE controls if the interface configuration is
 * fully settable. Prepare additional defines needed here:
 *   - IFACE_CFG_SET_MODE is used to initialize the set mode of the affected
 * attributes (3, 5, 6).
...
static bool IsValidNetworkConfig(const CipTcpIpInterfaceConfiguration* if_cfg) {
  ...
  if (!IsInClassAbc(if_cfg->ip_address) || /* ABC */
      !IsInClassAbc(if_cfg->gateway) || !IsInClassAbc(if_cfg->name_server) ||
