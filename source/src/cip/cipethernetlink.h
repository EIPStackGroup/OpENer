...
typedef enum {
  /** Set this bit if your device needs a reset to take over new settings made
   * via attribute #6. It is duplicates the meaning of kEthLinkCapManualReset */
  kEthLinkFlagsManualReset = 0x20,
} CipEthLinkIfaceFlags;

/** @brief Provide values for the Interface Control (attribute #6) Control bits
 * member */
typedef enum {
  /** Set this bit to enable Auto-negotiation of ethernet link parameters. */
  kEthLinkIfCntrlAutonegotiate = 0x01,
  /** When Auto-negotiation is disabled set this bit to force Full-Duplex mode
   * else Half-Duplex mode is forced. */
...
/** @brief Provide bit masks for the Interface Capability (#11) attribute's
 * Capability Bits */
typedef enum {
 ...
 *  This is attribute is becomes required if the HC Interface Counters attribute
 * or Media Counters attribute is implemented, otherwise highly recommended.
 ...
 *
 *  @return kEipStatusOk if initialization was successful, otherwise
 * kEipStatusError
 */
