 ...
 *
 * The comparison is done according to the definitions in the CIP specification
 * Section 3-5.5.2: The following elements have to be equal: Vendor ID,
 * Connection Serial Number, Originator Serial Number
 * @param connection_object connection object containing the comparison elements
 * from the forward open request
 * @return
 ...
 * @param key_data pointer to the electronic key data received in the forward
 * open request
 ...
 * @param connection_object pointer to the connection object structure for which
 * the connection should be established
 * @param message_router_request pointer to the received request structure. The
 * position of the data stream pointer has to be at the connection length entry
...
EipStatus ConnectionManagerInit(EipUint16 unique_connection_id) {
  ...
  InsertService(connection_manager,
                ...
                "GetAttributeSingle");
  InsertService(
    connection_manager, kGetAttributeAll, &GetAttributeAll, "GetAttributeAll");
  ...
  InsertService(connection_manager,
                ...
                "LargeForwardOpen");
  InsertService(
    connection_manager, kForwardClose, &ForwardClose, "ForwardClose");
  InsertService(connection_manager,
                kGetConnectionOwner,
                &GetConnectionOwner,
                "GetConnectionOwner");
      ...
      if (g_common_packet_format_data_item.data_item.type_id ==
        ...
        if (connection_object->originator_address.sin_addr.s_addr ==
          ...
          if (SEQ_GT32(g_common_packet_format_data_item.address_item.data
            ...
            /* only inform assembly object if the sequence counter is greater or
             * equal */
            connection_object->eip_level_sequence_count_consuming =
          ...
          OPENER_TRACE_WARN(
            "Connected Message Data Received with wrong address "
            "information\n");
        }
      }
    }
 ...
 *
 * Non-Null, Matching request - Return General Status =
 * kCipErrorConnectionFailure, Extended Status =
...
EipStatus HandleNonNullNonMatchingForwardOpenRequest(
  ...
  EipUint32 temp = ParseConnectionPath(
    &g_dummy_connection_object, message_router_request, &connection_status);
  ...
  if (kEipStatusOk != temp) {
    ...
    /* in case of success the new connection is added at the head of the
     * connection list */
 ...
 *
 * File scope variable
 * The first dimension handles if the request was a non-null request (0) or a
 * null request (1), the second dimension handles if the request was a
 * non-matchin (0) or matching request (1)
 */
 ...
 *  Non-Null connection - one or both O->T and T->O connection parameter field
 * are not null Matching - Connection Triad matches an existing connection
 *  (Connection Serial Number, Originator Vendor ID and Originator Serial
 * Number)
 *
 ...
 *  @param encapsulation_session associated encapsulation session of the
 * explicit message
...
EipStatus ForwardOpenRoutine(CipInstance* instance,
  ...
  bool is_matching_request =
    false; /* 1 = Matching Request, 0 = Non-Matching Request  */
...
EipStatus ForwardClose(CipInstance* instance,
  ...
  /* check connection_serial_number && originator_vendor_id &&
   * originator_serial_number if connection is established */
  ...
  /* set AddressInfo Items to invalid TypeID to prevent assembleLinearMsg to
   * read them */
  ...
  message_router_request->data +=
    2; /* ignore Priority/Time_tick and Time-out_ticks */
  ...
  EipUint16 originator_vendor_id =
    GetUintFromMessage(&message_router_request->data);
  EipUint32 originator_serial_number =
    GetUdintFromMessage(&message_router_request->data);
...
EipStatus SearchConnectionData(
  ...
  OPENER_TRACE_INFO(
    "SearchConnectionData for ConnSerNo: %d, OrigVendId: %d, OrigSerNo: "
    "%i,\n",
    Connection_serial_number,
    Originator_vendor_id,
    Originator_serial_number);
        ...
        if ((0 != ConnectionObjectGetExpectedPacketRate(connection_object)) &&
          ...
          if (connection_object->transmission_trigger_timer <= elapsed_time) {
            ...
            if (kConnectionObjectTransportClassTriggerProductionTriggerCyclic !=
                ConnectionObjectGetTransportClassTriggerProductionTrigger(
                  connection_object)) {
              /* non cyclic connections have to reload the production inhibit
               * timer */
 ...
 * @param common_data_packet_format_data The CPF data packet where the Null
        ...
        if (kLogicalSegmentSpecialTypeLogicalFormatElectronicKey ==
          ...
          if (kElectronicKeySegmentFormatKeyFormat4 ==
            ...
            OPENER_TRACE_INFO(
              ...
              "%d\n",
              ElectronicKeyFormat4GetVendorId(
...
EipStatus AddConnectableObject(
  const CipUdint class_code, OpenConnectionFunction open_connection_function) {
