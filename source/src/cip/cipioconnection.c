 ...
 * It will generate a new IO connection based on the data parsed in the Forward
 * Open service
 *
...
CipError EstablishIoConnection(
  ...
  *extended_error = ProcessProductionInhibitTime(io_connection_object);
  ...
  /* we don't need to check for zero as this is handled in the connection path
   * parsing */
...
EipStatus OpenMulticastConnection(
  ...
  if (kCipItemIdSocketAddressInfoTargetToOriginator ==
    ...
    /* we are using an unused item initialize it with the default multicast
     * address */
...
EipStatus SendConnectedData(CipConnectionObject* connection_object) {
  ...
  if (s_produce_run_idle && !is_heartbeat) {
    AddDintToMessage(g_run_idle_state, &outgoing_message);
  }
