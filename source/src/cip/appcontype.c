 ...
 *
 * @param connection_object Connection Object which will be searched for in the
 * Exclusive Owner Connections
 * @param extended_error Pointer to the extended error variable, will be written
 * if an error occurs
...
void CloseAllConnectionsForInputWithSameType(
  ...
  while (NULL != node) {
    ...
    if ((instance_type == ConnectionObjectGetInstanceType(connection)) &&
      ...
      CheckIoConnectionEvent(connection_to_delete->consumed_path.instance_id,
...
void InitializeIoConnectionData(void) {
  ...
  memset(g_listen_only_connections,
         0,
