	...
	if ((dwRetVal = GetAdaptersInfo(pAdapterInfo, &ulOutBufLen)) == NO_ERROR) {
		...
		while (pAdapter) {
			if (pAdapter->Index == interface_index) {		
				...
				interface_configuration_.ip_address = inet_addr(pAdapter->IpAddressList.IpAddress.String);
		...
		while (pCurrAddresses) {
			if (interface_index == pCurrAddresses->IfIndex) {
				...
				if (i != 0) {
					...
					InetNtop(AF_INET, pCurrAddresses->FirstDnsServerAddress->Address.lpSockaddr->sa_data + 2, interface_configuration_.name_server, sizeof(interface_configuration_.name_server));
