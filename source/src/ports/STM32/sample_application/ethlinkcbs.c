...
/*                               INCLUDES                                    */
/*---------------------------------------------------------------------------*/
#include "ports/STM32/sample_application/ethlinkcbs.h"
...
EipStatus EthLnkPreGetCallback(CipInstance* const instance,
  ...
  switch (attr_no) {
    ...
    case 5: {
       ...
       * that expects the media counters to be zero after a GetAndClear service.
  ...
  if (hadAction) {
    OPENER_TRACE_INFO("Eth Link PreCallback: %s, i %" PRIu32 ", a %" PRIu16
                      ", s %" PRIu8 "\n",
...
EipStatus EthLnkPostGetCallback(CipInstance* const instance,
  ...
  if (kEthLinkGetAndClear == (service & 0x7f)) {
    OPENER_TRACE_INFO("Eth Link PostCallback: %s, i %" PRIu32 ", a %" PRIu16
                      ", s %" PRIu8 "\n",
                      instance->cip_class->class_name,
                      inst_no,
                      attribute->attribute_number,
                      service);
    /* Clear the instance specific object counters. In this dummy function we
     * only clear our GetAttributeSingle PreCallback execution counters. */
    switch (attribute->attribute_number) {
      ...
      case 5:
        ...
        break;
      default:
        OPENER_TRACE_INFO("Wrong attribute number %" PRIu16
                          " in GetAndClear callback\n",
                          attribute->attribute_number);
        break;
    }
  }
