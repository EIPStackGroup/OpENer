...
int NvQosLoad(CipQosObject *p_qos) {
  ...
  /*
   * Data type for these parameters explicitly differs from the
   * specification(CipUsint), because the required scanf specifier to read that
   * type, "hhu", is not supported in MinGW. The generic unsigned type is used
   * instead, which is guaranteed to fit a CipUsint, followed by a check after
   * scanf to ensure valid values before casting them to CipUsint upon
   * assignment to the output structure.
   */
...
#pragma warning(default : 4996)
  ...
  if (kEipStatusOk == eip_status) {
    /*
     * Ensure all values read from the configuration file are within the
     * CipUsint range; see above comments for these dscp_* local variables for
     * details.
     */
    if ((dscp_urgent > UCHAR_MAX) || (dscp_scheduled > UCHAR_MAX) ||
        (dscp_high > UCHAR_MAX) || (dscp_low > UCHAR_MAX) ||
