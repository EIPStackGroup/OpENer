 ...
 *
 *  @return EipStatusOk if initialization was successful, otherwise
 * EipStatusError
 */
...
/** @brief Returns current time in microseconds from monotonic time base, please
 * note that this does not represent a real absolute time, but measured from an
 * arbitrary starting point
 *
 * This function returns the current time relative to an arbitrary starting
 * point from a monotonic time source. As monotonic clocks and clock functions
 * in general are platform dependent, this has to be implemented for each
 * platform (see ports subfolders)
 *
 *  @return Current time relative to monotonic clock starting point as
 * MicroSeconds
 */
MicroSeconds GetMicroSeconds(void);

/** @brief Returns current time in milliseconds from monotonic time base, please
 * note that this does not represent a real absolute time, but measured from an
 * arbitrary starting point
 *
 * This function returns the current time relative to an arbitrary starting
 ...
 *
 * A wrapper function - needs a platform dependent implementation to set QoS on
 * a socket
 *
