 ...
 *
 *  The generic network handler delegates platform-dependent tasks to the
...
#define MSG_NOSIGNAL_PRAGMA_MESSAGE                                           \
  "MSG_NOSIGNAL not defined. Check if your system stops on SIGPIPE, as this " \
  "can happen with the send() function"
#if defined(_WIN32)
...
/** @brief Checks and processes request received via the UDP unicast socket,
 * currently the implementation is port-specific
 *
 */
...
/** @brief check if on the UDP consuming socket data has been received and if
 * yes handle it correctly
 *
 */
void CheckAndHandleConsumingUdpSocket(void);

/** @brief Handles data on an established TCP connection, processed connection
 * is given by socket
 *
...
EipStatus NetworkHandlerProcessCyclic(void) {
  ...
  /* check if we had been not able to update the connection manager for several
   * kOpenerTimerTickInMilliSeconds. This should compensate the jitter of the
   * windows timer
   */
  if (g_network_status.elapsed_time >= kOpenerTimerTickInMilliSeconds) {
    /* call manage_connections() in connection manager every
     * kOpenerTimerTickInMilliSeconds ms */
  ...
  if ((unsigned)number_of_read_bytes == data_size) {
    ...
    if (need_to_send > 0) {
      ...
      if (data_sent != outgoing_message.used_message_length) {
        OPENER_TRACE_WARN("TCP response was not fully sent: exp %" PRIuSZT
                          ", sent %ld\n",
                          outgoing_message.used_message_length,
                          data_sent);
      }
...
/** @brief Create the UDP socket for the implicit IO messaging, one socket
