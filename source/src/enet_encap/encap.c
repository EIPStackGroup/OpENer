...
#define ENCAP_NUMBER_OF_SUPPORTED_DELAYED_ENCAP_MESSAGES                     \
  2 /**< According to EIP spec at least 2 delayed message requests should be \
       supported */
...
EipStatus HandleReceivedExplictTcpData(int socket,
  ...
  if (kEncapsulationHeaderOptionsFlag ==
      encapsulation_data.options) /*TODO generate appropriate error response*/
  {
    if (*number_of_remaining_bytes >=
        0) /* check if the message is corrupt: header size + claimed payload
              size > than what we actually received*/
    {
...
void HandleReceivedListIdentityCommandUdp(
  ...
  if (NULL != delayed_message_buffer) {
    ...
    memcpy((&delayed_message_buffer->receiver),
           from_address,
...
CipUint ListIdentityGetCipIdentityItemLength() {
  return sizeof(CipUint) + sizeof(CipInt) + sizeof(CipUint) + sizeof(CipUdint) +
         ...
         sizeof(CipUint) + 2 * sizeof(CipUsint) + sizeof(CipWord) +
         sizeof(CipUdint) + sizeof(CipUsint) + g_identity.product_name.length +
         sizeof(CipUsint);
}
...
void DetermineDelayTime(
  ...
  if (0 == maximum_delay_time) {
    maximum_delay_time = kListIdentityDefaultDelayTime;
             ...
             maximum_delay_time) { /* if maximum_delay_time is between 1 and
                                      500ms set it to 500ms */
 ...
 * originator.
 * @param socket Socket this request is associated to. Needed for double
 * register check
 ...
 * @param receive_buffer_length Length of the data in receive_buffer. Might be
 ...
 *              kInvalidSession .. invalid session -> return unsupported command
 * received
 */
SessionStatus CheckRegisteredSessions(
  const EncapsulationData* const receive_data) {
