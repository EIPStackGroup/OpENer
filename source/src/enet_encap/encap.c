...
void HandleReceivedListIdentityCommandUdp(const socket_platform_t socket, const struct sockaddr_in *const from_address, const EncapsulationData *const receive_data);
...
EipStatus HandleReceivedExplictTcpData(socket_platform_t socket, EipUint8 *buffer, size_t length, int *number_of_remaining_bytes, struct sockaddr *originator_address,
    ENIPMessage *const outgoing_message) {
...
EipStatus HandleReceivedExplictUdpData(const socket_platform_t socket, const struct sockaddr_in *from_address, const EipUint8 *buffer, const size_t buffer_length,
