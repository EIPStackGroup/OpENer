...
void ConfigureMacAddress(
  const EipUint8 *const mac_address);
...
void ConfigureDomainName(
  const char *const RESTRICT domain_name);
...
void ConfigureHostName(
  const char *const RESTRICT host_name);
 ...
 */
void CipStackInit(
  const EipUint16 unique_connection_id);
 ...
 *          0 if instance is not in the object
 */
CipAttributeStruct *GetCipAttribute(
  const CipInstance *const cip_instance,
 ...
 *
 */
CipInstance *AddCIPInstance(
  CipClass *RESTRICT const cip_class_to_add_instance,
...
void InsertAttribute(
  CipInstance *const cip_instance,
  const EipUint16 attribute_number,
 ...
 *
 */
void AllocateAttributeMasks(
  CipClass *target_class);
 ...
 */
void InsertService(
  const CipClass *const cip_class_to_add_service,
 ...
 */
CipInstance *CreateAssemblyObject(
  const EipUint32 instance_number,
  EipByte *const data,
 ...
 */
int HandleReceivedExplictTcpData(
  int socket,
  EipUint8 *buffer,
