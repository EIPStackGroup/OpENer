...
TEST(CipCommon, DecodeCipString) {
  ...
  for (size_t i = 0; i < number_of_strings; i++) {
    ...
    // check string + length
    CHECK_EQUAL(*(data + pos_in_data),
                string.length);  // first element at current pos contains the
                                 // length of the following string
    MEMCMP_EQUAL(data + pos_in_data + 2,
                 string.string,
                 string.length);  // pos_in_data + 2 bytes for length
...
TEST(CipCommon, DecodeCipShortString) {
  ...
  for (size_t i = 0; i < number_of_strings; i++) {
    ...
    // check string + length
    CHECK_EQUAL(*(data + pos_in_data),
                short_string.length);  // first element at current pos contains
